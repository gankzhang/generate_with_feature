...
def discriminator(x_onehot,encoder_state, batch_size, seq_len, vocab_size,_embed_ph,D_with_state = param.D_with_state,filter_dim = 100):
...
#    _embed_init = d_embeddings.assign(_embed_ph)
    ...
    if D_with_state:
        num_filters_total = sum(num_filters)+int(encoder_state.shape[-1])
    else:
        num_filters_total = sum(num_filters)
    h_pool = tf.concat(pooled_outputs, 3)  # batch_size x 1 x num_rep x num_filters_total

    if D_with_state:
        h_pool = tf.concat([tf.reshape(tf.tile(encoder_state, [1, num_rep, 1]), [BATCH_SIZE,1,num_rep,int(encoder_state.shape[-1])]), h_pool], 3)
